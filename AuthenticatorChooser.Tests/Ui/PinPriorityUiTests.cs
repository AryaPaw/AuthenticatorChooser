...
public sealed class PinPriorityUiTests {
    ...
    public void StatusForm_CacheModeStoresWithoutPersistingPin() {
        RunSta(() => {
            ...
            Flatten(form).OfType<Label>().Single(l => l.AccessibleName == "pinCacheValueLabel").Text.Should().Be("PIN");
            Flatten(form).OfType<Label>().Single(l => l.AccessibleName == "pinCacheConfirmLabel").Text.Should().Be("Confirm PIN");
            Flatten(form).OfType<Label>().Select(l => l.Text).Should().Contain(t => t.Contains("twice"));
            TextBox pin = Flatten(form).OfType<TextBox>().Single(t => t.AccessibleName == "pinCacheValue");
            TextBox confirm = Flatten(form).OfType<TextBox>().Single(t => t.AccessibleName == "pinCacheConfirm");
            pin.Text = "13579";
            confirm.Text = "13579";
            Flatten(form).OfType<Button>().Single(b => b.AccessibleName == "pinCacheStore").PerformClick();
            pin.Text.Should().BeEmpty();
            confirm.Text.Should().BeEmpty();
            cache.HasCached.Should().BeTrue();
            File.ReadAllText(settings).Should().NotContain("13579");
