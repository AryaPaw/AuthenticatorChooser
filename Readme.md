...
[![Latest release](https://img.shields.io/github/v/release/AryaPaw/AuthenticatorChooser?include_prereleases&logo=github)](https://github.com/AryaPaw/AuthenticatorChooser/releases/latest)
...
This is an independent continuation of [Aldaviva/AuthenticatorChooser](https://github.com/Aldaviva/AuthenticatorChooser). The original idea is the same: skip the phone pairing screen and choose a USB security key in Windows FIDO/WebAuthn prompts. This fork adds a status window, a tray icon, settings, and autostart, because the upstream project has not been updated in a long time.
...
1. [Download the latest release](https://github.com/AryaPaw/AuthenticatorChooser/releases/latest). Use `AuthenticatorChooser-win-x64.exe` on most PCs, or `AuthenticatorChooser-win-arm64.exe` on ARM laptops (Surface Pro X and similar).
1. Copy the file somewhere permanent, like `C:\Program Files\AuthenticatorChooser\`. You can rename it to `AuthenticatorChooser.exe`.
1. Double-click it. Windows will ask for administrator approval so the program can click the Windows Security prompt. It stays in the notification area (tray). Double-click the key icon to open the status window.
1. Leave **Start when I sign in** enabled if you want it to start with Windows.

## Status window and tray

The program starts in the tray. The status window stays hidden until you double-click the key icon or choose Open.

- **Pause / Resume** — stop automatic clicks until you turn it back on (same idea as holding Shift, but it stays off)
- **Always choose the USB security key** — also skip Windows Hello and an already-paired phone, not only the “new phone” option
- **Start when I sign in** — start with Windows after you log on
- **Autosubmit PIN** — type a PIN of the length you actually use, then **Turn on**. Only the length is saved, not the digits. **Turn off** disables it. This is the USB-key PIN, not Windows Hello.
- **Debug log** — write a log under `%AppData%\AuthenticatorChooser`
- Closing the window keeps the tray icon. **Exit** quits the program.
- A second launch just opens the existing window instead of starting twice.

### Overriding the automatic next behavior

By default, this program does not interfere with local TPM passkey prompts (like requesting your Windows Hello PIN or biometrics). It also does not automatically submit FIDO prompts that contain additional options besides a USB security key and pairing a new Bluetooth smartphone, such as the cases when you already have a paired phone, or you previously declined a Windows Hello factor like a PIN but want to try a PIN again from the authenticator choice dialog. However, you may override this behavior if you wish and force it to **_always_** choose the USB security key in all cases, even if there are other valid options like Windows Hello PIN/biometrics, by enabling **Always choose the USB security key**.
...
To try a sample FIDO prompt, visit [WebAuthn.io](https://webauthn.io) and click **Authenticate**.

## Building

If you want to compile it yourself instead of using [the release](https://github.com/AryaPaw/AuthenticatorChooser/releases/latest):

1. Install the [.NET SDK](https://dotnet.microsoft.com/en-us/download) (8 or later).
1. Clone this repository and publish:
...
The exe is written to `AuthenticatorChooser\bin\Release\net8.0-windows\win-x64\publish\AuthenticatorChooser.exe`. Visual Studio Community 2022 or 2026 also works.

Original program © Ben Hutchison. This repository is an independent fork maintained by AryaPaw.

## Related

### Creating new passkeys
When you try to create a passkey in your browser, the website may force it to be stored only in the TPM or only on a security key, rather than letting you freely choose between the two destinations. To override the site's mandate and put yourself back in control of where your new passkey will be saved, you can install [**Create Passkeys Anywhere**](https://github.com/Aldaviva/userscripts/raw/master/create-passkeys-anywhere.user.js) (requires [Tampermonkey](https://tampermonkey.net/) or a similar browser extension). It doesn't only run on Windows, for example it also works on Firefox for Android.
